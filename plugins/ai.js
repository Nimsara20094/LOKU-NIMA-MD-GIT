...
cmd({
    ...
    try {
        ...
        // Handle empty response from API (success but empty result)
        if (response.data.success && (!response.data.result || response.data.result.trim() === "")) {
            return reply("❌ No response was generated by the API. Please try a different query or try again later.");
        }
        ...
        // If no valid response found, use the entire data object as fallback
        ...
        if (!gptResponse || gptResponse.trim().length < 1) {
            return reply("❌ Could not get a valid response from the API. Please try again later or use a different query.");
        }
